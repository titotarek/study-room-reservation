...
<div id="confirmCancelModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="confirmCancelTitle" aria-describedby="confirmCancelDescription" aria-hidden="true">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
        <button type="button"
                onclick="closeCancelModal()"
                aria-label="Close cancel reservation dialog"
                class="absolute top-3 right-3 text-slate-400 hover:text-slate-600">
            ✕
        </button>

        <h3 id="confirmCancelTitle" class="text-xl font-bold mb-4">Cancel Reservation</h3>
        <p id="confirmCancelDescription" class="text-sm text-slate-700 mb-6">
        ...
        <div class="flex justify-end gap-3">
            ...
            <form id="cancelReservationForm" action="/reservations/cancel" method="POST">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
                <input type="hidden" name="id" id="cancelReservationId">
                <button type="button"
                        onclick="confirmCancelReservation()"
                        class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 font-semibold">
                    Cancel Reservation
                </button>
            </form>
        </div>
